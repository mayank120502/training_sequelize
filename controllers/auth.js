...
const loginController = async (req, res) => {
  try {
    ...
    if (doesPasswordMatch) {
      ...
      await commonService.updateData(user, { email: found.email }, { token });
...
const resetPasswordController = async (req, res) => {
  try {
    ...
    if (found.status === '2') {
      return res.send(errorRes(403, 'Disabled User'));
    }
...
const getAllUsers = async (req, res) => {
  try {
    ...
    const data = await commonService.getDataAll1(user, condition, ['id', 'name', 'email', 'status'], limit, offset);
    if (!data) {
